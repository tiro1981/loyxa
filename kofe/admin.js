...
function initAdmin() {
    ...
    // Dark mode — Kofe'da standart qorong'u (faqat '0' bo'lsa yoritilgan)
    if (localStorage.getItem('kofe_dark') === '1') document.body.classList.add('dark');
    document.getElementById('darkToggle').onclick = () => {
        document.body.classList.toggle('dark');
        localStorage.setItem('kofe_dark', document.body.classList.contains('dark') ? '1' : '0');
        renderDashboard();
    };
...
function renderProducts() {
    ...
    if (pageList.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7"><div class="empty"><div class="empty-ico">📦</div>Mahsulot topilmadi</div></td></tr>`;
    } else {
...
window.deleteProduct = function(id) {
    const p = data.products.find(x => x.id === id);
    if (!p) return;
    confirmAction('Mahsulotni o\'chirish', `"${p.name}" mahsulotini o'chirmoqchimisiz? Bu amalni qaytarib bo'lmaydi.`, () => {
...
window.viewOrder = function(id) {
    ...
    document.getElementById('orderModalBody').innerHTML = `
        ...
        <h4 style="font-size:13px;text-transform:uppercase;color:var(--text-muted);margin:14px 0 8px;letter-spacing:0.05em">Mahsulotlar</h4>
            ...
            <div class="order-items-list">
                ${orders.slice(0, 6).map(o => `
                    <div class="oi">
                        ...
                        <div>
                            <div class="oi-name">${o.items.length} mahsulot</div>
    ...
    document.getElementById('catGrid').innerHTML = data.categories.map(c => {
        ...
        return `
            <div class="cat-card">
                ...
                <strong>${c.name}</strong>
                <small>${c.desc || ''}</small>
                <div class="cat-count">${count} mahsulot</div>
            </div>`;
    }).join('');
}
...
const BOT_CFG_KEY = 'kofe_bot_config';
...
function getBotApi() { return (localStorage.getItem('bo_bot_api') || localStorage.getItem('kofe_bot_http_url') || 'http://localhost:3344').replace(/\/+$/, ''); }
...
function _shopName() { try { const d = (typeof Store !== 'undefined') ? Store.load() : {}; return (d.settings && (d.settings.storeName || d.settings.shopName)) || 'Kofe'; } catch { return 'Kofe'; } }
