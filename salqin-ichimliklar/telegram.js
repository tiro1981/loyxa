// Salqin — do'kon boti (store-bot) API klienti.
// Har bir do'kon o'z bot tokenini admin paneldan ulaydi. Frontend buyurtmani
// store-bot serveriga (bot.py, port 3344) yuboradi, server esa kanalga uzatadi.
const Telegram = (() => {
  // Do'kon kaliti — dashboarddan ?client=... bo'lsa o'sha, aks holda 'shop'
  const SHOP_KEY = (new URLSearchParams(location.search).get('client') || (() => { try { return JSON.parse(localStorage.getItem('bo_session') || '{}').clientId; } catch { return null; } })() || 'shop') + '__salqin';
  // Bot server manzili (platforma sozlamasi bilan bir xil kalit)
  const API_URL = (localStorage.getItem('bo_bot_api') || localStorage.getItem('si_bot_http_url') || 'http://localhost:3344')
    .replace(/\/+$/, '');
...
// script.js `window.Telegram` orqali tekshirgani uchun aniq export qilamiz.
